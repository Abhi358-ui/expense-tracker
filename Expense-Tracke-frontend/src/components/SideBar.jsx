...
import logo5 from "../assets/zetron.png";
...
const SideBar = () => {
  const { token, setToken, fetchIncome, fetchExpense } = useContext(AppContext);
      ...
      <div className="bg-gradient-to-r from-black to-gray-700 h-screen">
        <div className="mt-3 py-2 px-2">
          ...
          <img
            ...
            onClick={() => navigate("/")}
            alt="logo"
            className="w-12 block hidden md:hidden"
          />
        </div>
        ...
        <div className="flex flex-row items-center lg:hover:bg-red-500 lg:border-none hover:border-l-2 border-red-500 justify-center gap-5 py-2 px-2">
          <NavLink
            to={"/"}
            className="w-full flex-row items-center justify-center gap-2 py-2 px-2 rounded-lg cursor-pointer"
          >
